<?php
  ...
  if(isset($_POST['submit'])){
    if(empty($_POST['reservationno']) || empty($_POST['passengerid']) || empty($_POST['driverid']) || empty($_POST['startplace']) || empty($_POST['endplace']) || empty($_POST['status'])){
      ...
      echo "<script> alert('Reservation successful.Please wait for the driver to reach your place!!'); </script>";      
    }
  }
...
<body>
              ...
              <div class="card-body">
                ...
                <form class="row g-3" action="" method="POST">
                  <div class="col-6">
                    ...
                    <input type="text" name="reservationno" class="form-control" value="<?php echo $new_rid; ?>" id="inputNanme4" readonly>
                  </div>

                  <div class="col-6">
                    <label for="inputState" class="form-label">Passenger ID</label>
                  ...
                  <div class="col-6">
                    ...
                    <input type="text" name="status" value="<?php echo $status; ?>" class="form-control" id="" autocomplete="off" readonly>
                  </div> 
                    ...
                    while($row=mysqli_fetch_array($table_result))
                    {
                     ?>
                       <tr>
                         ...
                         <td><a href="/city-taxi/phpmailer/send_booking.php?id=<?php echo $row['reservationno']; ?>"><button type="button" name="send" class="btn btn-primary">Send</button></a></td>
                        </tr>	
                      <?php
                    }
                    ?>                   
  ...
  <script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyAmy39RcNE3UWzQPZWISgssqsY_DTpnTpg&libraries=places"></script>
  
  <script>
    // Initialize the Google Places Autocomplete service
    var autocomplete1 = new google.maps.places.Autocomplete(document.getElementById('addressInput1'));
    var autocomplete2 = new google.maps.places.Autocomplete(document.getElementById('addressInput2'));
    // Restrict the autocomplete to addresses only
